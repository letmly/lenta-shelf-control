"""Прогон pipeline_v7 на всех 5 размеченных видео + evaluate + dashboard."""
...
for name, filename_in_csv in VIDEOS:
    ...
    out_csv = ROOT / f"outputs/pipeline_v7/{name}.csv"
    df_pred = process_video(video, fps_sample=3, output_csv=out_csv,
                             filename_in_csv=filename_in_csv, upscale=3.0)
    if df_pred is None or len(df_pred) == 0:
        results[name] = {"gt": 0, "pred": 0, "matched": 0,
                          "mean_score": 0, "qualified_80": 0}
        continue
    ...
    print(f"\n  >>> {name}: pred={len(df_pred)} gt={len(df_gt)} matched={matched}/{len(df_gt)} ({matched/len(df_gt):.0%}) mean={mean:.0%} qual={qual}/{len(df_gt)} ({qual/len(df_gt):.0%})")
