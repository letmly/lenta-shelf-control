"""Прогон pipeline_v8 на всех 5 видео + evaluate + summary."""
...
for name, fn in VIDEOS:
    ...
    out_csv = ROOT / f"outputs/pipeline_v8/{name}.csv"
    ...
    print(f"  >>> {name}: pred={len(df_pred)} gt={len(df_gt)} matched={matched}/{len(df_gt)} ({matched/len(df_gt):.0%}) mean={mean:.0%} qual={qual}/{len(df_gt)}")
...
(ROOT / "outputs/pipeline_v8/summary.json").write_text(
