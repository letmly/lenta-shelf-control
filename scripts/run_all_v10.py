"""Прогон pipeline_v10 на всех 5 видео + summary."""
...
for name, fn in VIDEOS:
    ...
    out_csv = ROOT / f"outputs/pipeline_v10/{name}.csv"
    ...
    if df_pred is None or len(df_pred) == 0:
        results[name] = {"gt":0, "pred":0, "matched":0, "mean":0, "qual":0}
        continue
    df_gt = pd.read_csv(ROOT / f"data/labeled/{name}/{name}.csv", encoding="utf-8")
    matches = match_rows(df_pred.copy(), df_gt.copy())
    scores = []
    for gi, pi, _ in matches:
        if pi is None: scores.append(0); continue
        s, _ = compare_fields(df_pred.iloc[pi], df_gt.iloc[gi])
        scores.append(s)
...
tot_qual = sum(r["qual"] for r in results.values())
print(f"GT: {tot_gt}  Matched: {tot_matched}/{tot_gt} ({tot_matched/tot_gt:.0%})  Qual: {tot_qual}/{tot_gt} ({tot_qual/tot_gt:.0%})")

(ROOT / "outputs/pipeline_v10/summary.json").write_text(
    json.dumps({"per_video": results, "total_gt": tot_gt, "total_matched": tot_matched,
